...
#TODO: SLIDING WINDOW Y PREDICCION
    def core(self, input_ds):
        ...
        if self.conf.plot_prefix != None:
            ...
            print("input_ds[(2 * self.conf.window_size) + self.conf.forward_ticks: self.rows_d-(2 * self.conf.window_size)), feature] = ", input_ds[(2 * self.conf.window_size) + self.conf.forward_ticks: self.rows_d-(2 * self.conf.window_size), feature].shape)
            ...
            for feature in range(self.cols_d):
                ...
                ax.plot(input_ds[(2 * self.conf.window_size) + self.conf.forward_ticks-1 : self.rows_d-(2 * self.conf.window_size), feature], lw=3, alpha=0.2, c='k', label='original')
                ax.legend()
