...
# versio c_v5 removes from the inputs the column used as training signal (shifted), so the pre-trainer does not use it as best approximation
...
def get_reward(action, window, min_TP, max_TP, min_SL, max_SL, min_dInv, max_dInv):
   ...
   # Continuous indicators = 6:rEMA, 7:rRSI, 8:rnEMA,9 rnRSI
    if action == 6:
        ...
        return {'reward':(window[11][5] - window[10][5])/0.0002, 'profit':0, 'dd':0 ,'min':0 ,'max':0, 'direction':0}
    if action == 7:
        # RETURN DE  RSI no normalizado (RSIf - RSI ACTUAL) ADELANTADO 1 día, strat: cierra en cambio de signo de pendiente 
        return {'reward':(window[1][5] - window[0][5])/0.0002, 'profit':0, 'dd':0 ,'min':0 ,'max':0, 'direction':0}
    if action == 8:
        ...
        rew = (window[11][5] - window[10][5])/0.0002
