...
def get_reward(action, window, min_TP, max_TP, min_SL, max_SL, min_dInv, max_dInv):
   ...
   #                            7:EMA(10)delayed 5 - EMA(40),
    if action == 6:
        # EMA(5)delayed 2 - EMA(10) : positive = buy
        reward = (window[2][17] - window[0][10])/0.005
        #if reward > 1.5:
        #    reward = 1.5
        #if reward <-1.5:
        #    reward = -1.5
    ...
    if action == 7:
        ...
        reward = (window[5][10] - window[0][24])/0.005
        #if reward > 1.5:
        #    reward = 1.5
        #if reward <-1.5:
        #    reward = -1.5
        return {'reward': reward, 'profit':0, 'dd':0 ,'min':0 ,'max':0, 'direction':0}
    if action == 8:
        # EMA(5)delayed 2 - EMA(20) : positive = buy
        reward = (window[2][17] - window[0][24])/0.005
        #if reward > 1.5:
        #    reward = 1.5
        #if reward <-1.5:
        #    reward = -1.5
