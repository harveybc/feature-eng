...
def get_reward(action, window, min_TP, max_TP, min_SL, max_SL, min_dInv, max_dInv):
   ...
   # Continuous indicators = 6:rEMA, 7:rRSI, 8:rnEMA,9 rnRSI
    if action == 6:
        ...
        return {'reward':(window[11][5] - window[10][5]), 'profit':0, 'dd':0 ,'min':0 ,'max':0, 'direction':0}
    if action == 7:
    ...
    if action == 8:
        ...
        return {'reward': discretize_reward(rew, 0.1, 1, -1), 'profit':0, 'dd':0 ,'min':0 ,'max':0, 'direction':rew}
    if action == 9:
