...
# version ae_v1 generates data with the decoder of a denoising autoencoder, this
# data can be used to train a network that directly controls a plant's actuators
# in this case the parameters of an order in each tick (nop, open buy, sell, close).

# it can also be used to train a control network that can be optimized with NEAT
# , an option to avoid overfitting is to use batch normalization for outputs of
# each layer.
...
def get_reward(action, window, min_TP, max_TP, min_SL, max_SL, min_dInv, max_dInv):
    ...
    # TODO: Reemplazar acciones por salida de encoder para entradas de entrenamiento de controlador con pretrainer
    # TODO: función para entrenar denoising autoencoder de tamaño variable
    # TODO: entrada adicional para tamaño de salida de encoder entrenado con denoising autoencoder
