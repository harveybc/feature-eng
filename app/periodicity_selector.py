...
- Fixes:
  * Robust NaN handling in z-scores (per-element fill with column median)
  * Safe ranks using na_option='bottom' and nullable Int64 dtype
  * One-time acronym legend + first-use expansions in logs
"""
...
def robust_zscore(x: np.ndarray) -> np.ndarray:
    """
    Robust z-score with per-element NaN safety:
    - Compute column median/MAD excluding NaN.
    - Replace NaN elements with median so they map to z=0.
    """
    x = np.asarray(x, dtype=float)
    med = np.nanmedian(x)
    mad = median_abs_deviation(x, nan_policy='omit', scale='normal')
    if np.isnan(med):
        # column is all-NaN -> return zeros
        return np.zeros_like(x, dtype=float)
    x_filled = np.where(np.isnan(x), med, x)
    if mad == 0 or np.isnan(mad):
        return np.zeros_like(x_filled, dtype=float)
    return (x_filled - med) / mad
...
def mutual_information_horizon(r: pd.Series, horizon_bars: int, k: int=5,
    ...
    if use_tqdm: rng = tqdm(rng, desc=f"Mutual Information (MI): h={horizon_bars} bars", leave=False, miniters=1)
...
def snr_smoothing_returns(r: pd.Series, smooth_window: int) -> float:
    """
    Signal-to-Noise Ratio (SNR) heuristic on returns:
      signal = centered moving average, noise = residual; supports small windows safely.
    """
    if (smooth_window is None) or (np.isnan(smooth_window)) or (smooth_window < 1):
        return np.nan
    x = pd.Series(r.dropna().values, index=r.dropna().index)
    if x.size < max(8, 4*smooth_window):
        return np.nan
    window = int(smooth_window)
    minp = min(window, max(1, window//3))
...
def permutation_entropy_weighted(r: pd.Series, m: int=5, tau: int=1, use_tqdm: bool=True) -> float:
    ...
    if use_tqdm: it = tqdm(it, desc="Weighted Permutation Entropy (WPE): encoding", leave=False, miniters=1)
    ...
    weights = np.maximum(weights / np.sum(weights), 1e-300)
...
def spectral_forecastability_bandlimited(r: pd.Series, band_hours: Tuple[float, float], rule: str) -> float:
    """
    Band-limited forecastability Ω:
      - Welch PSD on returns
      - keep only frequency bins whose period (hours) falls in band_hours
      - entropy over that band -> Ω_band = 1 - H_band / log(K_band)
    """
    x = np.asarray(r.dropna(), dtype=float)
    if x.size < 256: return np.nan
    ...
    p = Pxx[mask]
    p = np.maximum(p / np.sum(p), 1e-300)
...
def evaluate_periodicity(df_close: pd.DataFrame, rule: str,
    ...
    # First-use expansions in messages:
    ...
    HF_hours = [3.0, 4.0, 5.0, 6.0]         # High Frequency (HF) horizons in hours
    LF_hours = [72.0, 96.0, 120.0, 144.0]   # Long Frequency (LF) horizons in hours (3–6 days)

    # MI (Mutual Information) for all horizons in each band
    info(f"[{rule}] MI (Mutual Information) HF 3–6h…", quiet)
    mi_hf_vals = []
    ...
    info(f"[{rule}] MI (Mutual Information) LF 3–6d…", quiet)
    mi_lf_vals = []
    ...
    # Ω (Forecastability) band-limited
    info(f"[{rule}] Ω (Forecastability) en banda HF 3–12h…", quiet)
    omega_hf = spectral_forecastability_bandlimited(r, band_hours=(3.0, 12.0), rule=rule)
    info(f"[{rule}] Ω (Forecastability) en banda LF 2–10d…", quiet)
    omega_lf = spectral_forecastability_bandlimited(r, band_hours=(48.0, 240.0), rule=rule)

    # ACF (Autocorrelation Function) and SNR (Signal-to-Noise Ratio) at scale
    h_s_max = to_bars_or_nan(6.0, rule)
    h_l_max = to_bars_or_nan(144.0, rule)

    info(f"[{rule}] ACF_sum_abs (Autocorrelation mass) HF (≤6h)…", quiet)
    acf_hf = acf_sum_abs(r, int(h_s_max)) if (h_s_max==h_s_max) else np.nan
    info(f"[{rule}] ACF_sum_abs (Autocorrelation mass) LF (≤6d)…", quiet)
    acf_lf = acf_sum_abs(r, int(h_l_max)) if (h_l_max==h_l_max) else np.nan

    info(f"[{rule}] SNR (Signal-to-Noise Ratio) HF (ventana=6h)…", quiet)
    snr_hf = snr_smoothing_returns(r, int(h_s_max)) if (h_s_max==h_s_max) else np.nan
    info(f"[{rule}] SNR (Signal-to-Noise Ratio) LF (ventana=6d)…", quiet)
    snr_lf = snr_smoothing_returns(r, int(h_l_max)) if (h_l_max==h_l_max) else np.nan

    # WPE (Weighted Permutation Entropy) & LZ (Lempel–Ziv rate) – global penalties
    info(f"[{rule}] WPE (Weighted Permutation Entropy) global…", quiet)
    wpe = permutation_entropy_weighted(r, m=5, tau=1, use_tqdm=use_tqdm)
    info(f"[{rule}] LZ (Lempel–Ziv) rate global (método={lz_method})…", quiet)
    ...
    return {
        ...
        # raw MI (debugging)
        'mi_hf_h3_bits': mi_hf_vals[0] if len(mi_hf_vals)>0 else np.nan,
        'mi_hf_h4_bits': mi_hf_vals[1] if len(mi_hf_vals)>1 else np.nan,
        'mi_hf_h5_bits': mi_hf_vals[2] if len(mi_hf_vals)>2 else np.nan,
        'mi_hf_h6_bits': mi_hf_vals[3] if len(mi_hf_vals)>3 else np.nan,
        'mi_lf_d3_bits': mi_lf_vals[0] if len(mi_lf_vals)>0 else np.nan,
        'mi_lf_d4_bits': mi_lf_vals[1] if len(mi_lf_vals)>1 else np.nan,
        'mi_lf_d5_bits': mi_lf_vals[2] if len(mi_lf_vals)>2 else np.nan,
        'mi_lf_d6_bits': mi_lf_vals[3] if len(mi_lf_vals)>3 else np.nan,
    }
...
def composite_scores(df_metrics: pd.DataFrame, lambda_penalty: float) -> pd.DataFrame:
    ...
    # Ranks: put NaNs at bottom; use nullable Int64 dtype to avoid casting errors
    rank_hf = df_metrics['S_HF'].rank(ascending=False, method='dense', na_option='bottom')
    rank_lf = df_metrics['S_LF'].rank(ascending=False, method='dense', na_option='bottom')
    df_metrics['rank_HF'] = rank_hf.astype('Int64')
    df_metrics['rank_LF'] = rank_lf.astype('Int64')
    return df_metrics
...
def main():
    ...
    # One-time legend for acronyms:
    info((
        "[LEGEND] HF=High Frequency (3–6h); LF=Long Frequency (3–6d); "
        "MI=Mutual Information; Ω=Forecastability; "
        "ACF=Autocorrelation Function; SNR=Signal-to-Noise Ratio; "
        "WPE=Weighted Permutation Entropy; LZ=Lempel–Ziv rate."
    ), args.quiet)
    ...
    # Pretty print HF
    info("\n=== RESUMEN POR PERIODICIDAD — HF (High Frequency, 3–6h) ===", False)
    cols_hf = ['periodicity','n_points','mi_hf_med_bits','omega_hf','acf_hf_sum_abs','snr_hf','wpe','lz_entropy_rate','S_HF','rank_HF']
    dfr_hf = dfr.sort_values('S_HF', ascending=False).reset_index(drop=True)
    _fmt = lambda v: f"{v:,.6f}" if isinstance(v,(int,float,np.floating)) and not (isinstance(v,float) and (np.isnan(v))) else str(v)
    print(dfr_hf[cols_hf].to_string(index=True, float_format=_fmt), flush=True)

    # Pretty print LF
    info("\n=== RESUMEN POR PERIODICIDAD — LF (Long Frequency, 3–6d) ===", False)
    cols_lf = ['periodicity','n_points','mi_lf_med_bits','omega_lf','acf_lf_sum_abs','snr_lf','wpe','lz_entropy_rate','S_LF','rank_LF']
